...
const validateAssessmentData = (assessment: AssessmentData): string[] => {
  const errors: string[] = [];
...
const validateFrameworkData = (framework: Framework): string[] => {
  const errors: string[] = [];
...
const optimizeLocalStorage = (): void => {
  try {
    // Check storage usage
    let totalSize = 0;
    ...
    if (totalSize > 4 * 1024 * 1024) {
      logger.warn('LocalStorage approaching limit, cleaning up old data');
