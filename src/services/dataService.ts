...
export class DataService {
  ...
  private performDataMigration(fromVersion: string | null): void {
    logger.info(`Migrating data from version ${fromVersion || 'unknown'} to ${this.CURRENT_VERSION}`);
    ...
    try {
      ...
      const migratedAssessments = oldAssessments.map(assessment => ({
        ...
        evidenceLibrary: assessment.evidenceAssessment || [],
  ...
  deleteAssessment(id: string): void {
    try {
      ...
      throw new Error('Failed to delete assessment from storage');
    }
  }
  ...
  exportAssetsWithClassification(): string {
    try {
      ...
      logger.error('Failed to export assets with classification:', error);
    ...
    try {
      ...
      if (data.tasks && Array.isArray(data.tasks)) {
        ...
        const tasks = data.tasks.map(task => ({
          ...
          completedAt: task.completedAt ? new Date(task.completedAt) : undefined,
          approvedAt: task.approvedAt ? new Date(task.approvedAt) : undefined,
          // Add default values for new CMMC fields if they don't exist
          producedEvidenceIds: task.producedEvidenceIds || [],
          requiredEvidenceIds: task.requiredEvidenceIds || [],
          cmmcDomain: task.cmmcDomain || '',
          cmmcControl: task.cmmcControl || '',
          cuiImpact: task.cuiImpact || 'low',
          deliverables: task.deliverables || [],
          segregationOfDuties: task.segregationOfDuties || false,
          sspSection: task.sspSection || '',
          nistSP800171Mapping: task.nistSP800171Mapping || [],
          metadata: {
            ...task.metadata,
            cmmcImpact: task.metadata?.cmmcImpact || [],
            c3paoRequired: task.metadata?.c3paoRequired || false
          }
        }));
        this.saveTasks(tasks);
      }
    ...
    try {
      ...
      if (data.checksum) {
        ...
        if (checksum !== calculatedChecksum) {
          logger.warn('Backup checksum mismatch - data may be corrupted');
        }
      }
  ...
  loadDemoData(): void {
    try {
      ...
      const demoAssessment: AssessmentData = {
        ...
        frameworkId: 'cmmc',
        frameworkName: 'CMMC 2.0 Level 2 - Demo Assessment',
        responses: {
          'ac.1.001': 2,
          'ac.1.002': 1,
          'ac.2.005': 2,
        ...
        questionNotes: {
          'ac.1.001': 'Basic access controls implemented but need MFA for CUI systems.',
          'ma.2.001': 'Maintenance procedures exist but not fully documented per CMMC requirements.',
          'ps.1.001': 'Personnel security screening in progress for CUI access.'
        },
        questionEvidence: {},
        evidenceLibrary: [],
      ...
      const demoAssets = [
        {
          ...
          description: 'Main production web server hosting CUI applications for Military contracts',
          category: 'hardware',
          subcategory: 'server',
          type: 'server',
          ...
          cuiCategory: [
            {
              category: 'basic',
              subcategory: 'Technical Data',
              markings: ['CUI'],
              handlingRequirements: ['Encryption at rest', 'Access control', 'Audit logging']
            }
          ],
          cmmcApplicability: {
            level: 2,
            domains: ['AC', 'SC', 'SI'],
            controls: ['AC.2.001', 'SC.2.001', 'SI.2.001'],
            maturityProcesses: ['Access control management', 'System monitoring'],
            assessmentScope: 'full'
          },
          cuiScope: {
            inScope: true,
            scopeJustification: 'Processes technical data for Military contracts',
            boundaryDefinition: 'Dedicated CUI network segment',
            dataTypes: ['Technical Data', 'Contract Information', 'Design Specifications'],
            systems: ['Web Application', 'Database Server'],
            networks: ['CUI Network Segment']
          },
          dataFlow: {
            inputSources: [
              {
                assetId: 'demo-asset-002',
                connectionType: 'network',
                dataTypes: ['Technical Data'],
                cuiInvolved: true,
                encryptionStatus: 'encrypted',
                accessControls: ['MFA', 'VPN', 'Role-based access']
              }
            ],
            outputDestinations: [],
            dataAtRest: {
              ...
              accessControls: ['Database authentication', 'Application-level access'],
              backupProcedures: ['Nightly encrypted backups'],
              retentionPeriod: '7 years per contract requirements'
            },
            dataInTransit: {
              transportMethods: ['HTTPS', 'VPN'],
              encryptionProtocols: ['TLS 1.3'],
              networkSegmentation: true,
              authorizedPaths: ['CUI network segment only']
            },
            dataInUse: {
              processingControls: ['Application-level encryption', 'Secure processing'],
              userAccessControls: ['Role-based access', 'MFA'],
              sessionManagement: ['Session timeout', 'Secure session tokens'],
              dataHandlingProcedures: ['CUI handling procedures', 'Data classification']
            }
          },
          location: {
            type: 'physical',
      ...
      const demoTasks = [
        {
          id: 'demo-task-001',
          title: 'Complete CUI Asset Inventory Review',
          description: 'Review and update the comprehensive asset inventory to ensure all CUI systems and assets are properly documented and classified per CMMC requirements',
          ...
          producedEvidenceIds: ['ev-am-001', 'ev-am-002'],
          requiredEvidenceIds: ['ev-network-diagram'],
          cuiImpact: 'high',
          deliverables: ['Updated asset inventory', 'CUI data flow diagram', 'Asset classification matrix'],
          segregationOfDuties: true,
          sspSection: 'Section 3.2 - Asset Management',
          nistSP800171Mapping: ['3.4.1', '3.4.2'],
          tags: ['demo', 'asset-management', 'quarterly'],
          metadata: {
            cuiImpact: 'high',
            technicalComplexity: 'medium',
            riskReduction: 15,
            cmmcImpact: ['CMMC 2.0 Level 2'],
            successCriteria: ['CUI asset inventory updated', 'Asset classifications verified', 'CMMC scoping documented'],
            deliverables: ['Updated asset inventory', 'CUI data flow diagram', 'Asset classification matrix'],
            segregationOfDuties: true,
            c3paoRequired: true
          }
  ...
  async syncWithAPI(): Promise<void> {
    try {
      // Sync assessments
      const apiAssessments = await apiService.getAssessments();
      if (apiAssessments.length > 0) {
        this.saveAssessments(apiAssessments);
      }

      // Sync evidence
      const apiEvidence = await apiService.getEvidence();
      if (apiEvidence.length > 0) {
        // Update evidence in localStorage
        localStorage.setItem('evidence-data', JSON.stringify(apiEvidence));
      }

      logger.info('Data synced with API successfully');
    } catch (error) {
      logger.error('Failed to sync with API:', error);
      // Don't throw - continue with local data
    }
  }

  async saveAssessmentToAPI(assessment: AssessmentData): Promise<AssessmentData> {
    try {
      const savedAssessment = await apiService.saveAssessment(assessment);
      this.saveAssessment(savedAssessment);
      return savedAssessment;
    } catch (error) {
      logger.error('Failed to save assessment to API:', error);
      // Fallback to local storage
      return this.saveAssessment(assessment);
    }
  }

  async uploadEvidenceToAPI(evidence: FormData): Promise<{ id: string; url: string }> {
    try {
      const result = await apiService.uploadEvidence(evidence);
      return result;
    } catch (error) {
      logger.error('Failed to upload evidence to API:', error);
      throw error;
    }
  }

  // Hybrid methods that try API first, fallback to local storage
  async getAssessmentsHybrid(): Promise<AssessmentData[]> {
    try {
      const apiAssessments = await apiService.getAssessments();
      if (apiAssessments.length > 0) {
      ...
      logger.warn('API assessments not available, using local storage');
    }
    
    return this.getAssessments();
  }

  async getEvidenceHybrid(): Promise<any[]> {
    try {
      const apiEvidence = await apiService.getEvidence();
      if (apiEvidence.length > 0) {
        return apiEvidence;
      ...
      logger.warn('API evidence not available, using local storage');
    }
    
    const evidenceData = localStorage.getItem('evidence-data');
    return evidenceData ? JSON.parse(evidenceData) : [];
  }
}
