...
export class ReportService {
  ...
  private async exportToPDF(
    ...
    if ('showSaveFilePicker' in window) {
      try {
        ...
        logger.warn('Native PDF API failed, falling back to print method');
      }
    }
  ...
  private generateHTMLReport(
    ...
    const logoUrl = options.branding?.logo ? `${window.location.origin}${options.branding.logo}` : null;
    ...
    return `
      ...
      <html>
        ...
        <body>
          <div class="header">
            ${logoUrl ? `<img src="${logoUrl}" alt="Organization Logo" style="max-height: 60px; margin-bottom: 15px;" />` : ''}
          ...
          <div class="footer">
            <p>Report generated by CyberCertitude™ Cybersecurity Compliance Platform</p>
  ...
  private generatePDFWithPrint(htmlContent: string, assessment: AssessmentData, framework: Framework): void {
    ...
    // Use safer method to set document content
