...
import { 
  CheckSquare, AlertTriangle, Plus, Search,
  Calendar, Target,
  Eye, Edit3, Trash2, MessageSquare, Paperclip,
  CheckCircle, XCircle
} from 'lucide-react';
...
export const TaskManagementDashboard: React.FC<TaskManagementDashboardProps> = ({
  ...
  const [taskFormData, setTaskFormData] = useState({
    ...
    framework: 'SP 800-171' as string
  });
  ...
  const metrics: TaskMetrics = useMemo(() => {
    ...
    const overdueTasks = tasks.filter((t: Task) => new Date(t.dueDate) < new Date() && t.status !== 'completed').length;
    
    const tasksByStatus = tasks.reduce((acc: Record<TaskStatus, number>, task: Task) => {
      acc[task.status] = (acc[task.status] || 0) + 1;
      return acc;
    }, {} as Record<TaskStatus, number>);

    const tasksByPriority = tasks.reduce((acc: Record<TaskPriority, number>, task: Task) => {
      acc[task.priority] = (acc[task.priority] || 0) + 1;
      return acc;
    }, {} as Record<TaskPriority, number>);

    const tasksByFunction = tasks.reduce((acc: Record<string, number>, task: Task) => {
  ...
  const kanbanColumns = [
    { id: 'not-started', title: 'Not Started', tasks: filteredTasks.filter((t: Task) => t.status === 'not-started') },
    { id: 'in-progress', title: 'In Progress', tasks: filteredTasks.filter((t: Task) => t.status === 'in-progress') },
    { id: 'blocked', title: 'Blocked', tasks: filteredTasks.filter((t: Task) => t.status === 'blocked') },
    { id: 'review', title: 'Review', tasks: filteredTasks.filter((t: Task) => t.status === 'review') },
    { id: 'completed', title: 'Completed', tasks: filteredTasks.filter((t: Task) => t.status === 'completed') }
  ];
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    const newTask: Task = {
      ...
      tags: taskFormData.tags ? taskFormData.tags.split(',').map((t: string) => t.trim()).filter(Boolean) : [taskFormData.type, taskFormData.nistFunction.toLowerCase()],
      producedEvidenceIds: [],
      requiredEvidenceIds: [],
      deliverables: [],
      ...
      metadata: {
        ...
        riskReduction: 10,
        complianceImpact: [taskFormData.framework || 'SP 800-171'],
        successCriteria: ['Task completed on time', 'Deliverables approved'],
        cmmcImpact: [],
        selfAssessmentRequired: false
      }
    };
  ...
  const handleUpdateTaskStatus = async (taskId: string, newStatus: TaskStatus) => {
    ...
    try {
      ...
      setTasks((prev: Task[]) => prev.map((t: Task) => t.id === taskId ? updatedTask : t));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-300">
                  Track compliance implementation tasks and deliverables across Privacy, CMMC, and SP 800-171 frameworks
                </p>
              </div>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 mb-8 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <select
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFilters((prev: TaskFilter) => ({ 
              ...prev, 
          ...
          <select
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFilterAssignee(e.target.value)}
            className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
          >
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setTaskFormData(prev => ({ ...prev, description: e.target.value }))}
                  rows={3}
                  ...
                  placeholder="Describe the compliance task objectives and deliverables (Privacy, CMMC, or SP 800-171)"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setTaskFormData(prev => ({ ...prev, type: e.target.value as TaskType }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setTaskFormData(prev => ({ ...prev, priority: e.target.value as TaskPriority }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  >
                    <option value="low">Low</option>
                    <option value="medium">Medium</option>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setTaskFormData(prev => ({ ...prev, nistFunction: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  >
                    <option value="Govern">Govern</option>
                    <option value="Identify">Identify</option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTaskFormData(prev => ({ ...prev, nistCategory: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTaskFormData(prev => ({ ...prev, nistSubcategory: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setTaskFormData(prev => ({ ...prev, assignedTo: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTaskFormData(prev => ({ ...prev, dueDate: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTaskFormData(prev => ({ ...prev, estimatedHours: parseInt(e.target.value) || 8 }))}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  />
                </div>
              </div>
