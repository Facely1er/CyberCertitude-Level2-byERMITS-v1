...
const RiskAssessmentGenerator: React.FC<RiskAssessmentGeneratorProps> = ({
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'assessed': return <Eye className="w-4 h-4 text-primary-500" />;
      case 'mitigated': return <CheckCircle className="w-4 h-4 text-success-500" />;
      case 'accepted': return <XCircle className="w-4 h-4 text-text-muted-light" />;
      default: return <AlertTriangle className="w-4 h-4 text-text-muted-light" />;
    }
  };
      ...
      <div className="bg-surface-light dark:bg-surface-dark rounded-xl shadow-lg">
        ...
        <div className="p-6 border-b border-support-light dark:border-support-dark">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-background-light dark:bg-surface-dark p-4 rounded-lg">
              <div className="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark">
                {assessment.risks.length}
              </div>
              <div className="text-sm text-text-secondary-light dark:text-text-secondary-dark">Total Risks</div>
            </div>
            <div className="bg-background-light dark:bg-surface-dark p-4 rounded-lg">
              <div className="text-2xl font-bold text-error-600">
                {assessment.risks.filter(r => r.riskLevel === 'very-high' || r.riskLevel === 'high').length}
              </div>
              <div className="text-sm text-text-secondary-light dark:text-text-secondary-dark">High/Critical</div>
            </div>
            <div className="bg-background-light dark:bg-surface-dark p-4 rounded-lg">
              <div className="text-2xl font-bold text-yellow-600">
                {assessment.risks.filter(r => r.riskLevel === 'medium').length}
              </div>
              <div className="text-sm text-text-secondary-light dark:text-text-secondary-dark">Medium</div>
            </div>
            <div className="bg-background-light dark:bg-surface-dark p-4 rounded-lg">
              <div className="text-2xl font-bold text-success-600">
                {assessment.risks.filter(r => r.riskLevel === 'low' || r.riskLevel === 'very-low').length}
              </div>
              <div className="text-sm text-text-secondary-light dark:text-text-secondary-dark">Low</div>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <span className="text-sm font-medium text-text-primary-light dark:text-text-secondary-dark">
              Overall Risk Level:
            </span>
          ...
          {assessment.risks.length === 0 ? (
            ...
            <div className="space-y-4">
              {assessment.risks.map((risk) => (
                <div key={risk.id} className="border border-support-light dark:border-support-dark rounded-lg p-4">
