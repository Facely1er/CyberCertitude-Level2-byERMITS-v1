...
import { Upload, FileText, Image, Shield, Award, CircleAlert as AlertCircle, CircleCheck as CheckCircle, Calendar, User, Tag, Eye, Trash2, Link, Plus, Search, ListFilter as Filter, Download, ExternalLink, Clock, Star, Flag, Archive, RefreshCw } from 'lucide-react';
...
export const EvidenceManager: React.FC<EvidenceManagerProps> = ({
  ...
  const getConfidentialityColor = (level: string) => {
    switch (level) {
      case 'public': return 'bg-success-100 dark:bg-success-900/30 text-success-800 dark:text-success-300';
      case 'internal': return 'bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-300';
  ...
  const getRelevanceColor = (relevance: string) => {
    switch (relevance) {
      case 'primary': return 'bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-300';
      case 'supporting': return 'bg-success-100 dark:bg-success-900/30 text-success-800 dark:text-success-300';
      case 'reference': return 'bg-support-light dark:bg-background-dark/30 text-text-primary-light dark:text-text-secondary-dark';
      default: return 'bg-support-light dark:bg-background-dark/30 text-text-primary-light dark:text-text-secondary-dark';
    }
  };
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadFile = async () => {
      try {
        // Use local file service instead of network request
        const fileResult = await fileService.uploadFile(file);
        ...
        // Create evidence item with local file reference
        const newEvidence: QuestionEvidence = {
          evidenceId: fileResult.id,
  ...
  return (
    <div className={`bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-6 border border-purple-200 dark:border-purple-800 ${className}`}>
            ...
            return (
              <div key={evidence.id} className="bg-surface-light dark:bg-surface-dark rounded-lg p-4 border border-purple-200 dark:border-purple-700 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex space-x-2">
                    <button className="p-2 text-text-muted-dark hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                      <Eye className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => onRemoveEvidence(questionId, evidence.id)}
                      className="p-2 text-text-muted-dark hover:text-error-600 dark:hover:text-error-400 transition-colors"
                    >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-support-light dark:border-support-dark rounded-lg bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-dark focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 border border-support-light dark:border-support-dark rounded-lg bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-dark focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"
                  rows={3}
      ...
      {showLibraryModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-surface-light dark:bg-surface-dark rounded-2xl p-8 max-w-4xl w-full mx-4 shadow-2xl border border-support-light dark:border-support-dark max-h-[80vh] overflow-hidden">
            <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex space-x-4 mb-6">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-support-light dark:border-support-dark rounded-lg bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-dark focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                />
              </div>
              <select
                value={filterType}
            ...
            <div className="max-h-96 overflow-y-auto space-y-3">
              {filteredLibrary.length > 0 ? (
                filteredLibrary.map((evidence) => {
                  const IconComponent = getEvidenceTypeIcon(evidence.type);
                  return (
                    <div key={evidence.id} className="border border-support-light dark:border-support-dark rounded-lg p-4 hover:bg-background-light dark:hover:bg-surface-dark/50 transition-colors">
                      <div className="flex items-start justify-between">
                        <div className="flex items-start space-x-3 flex-1">
                          <div className="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-lg">
                            <IconComponent className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                          ...
                          <div className="flex-1">
                            <h5 className="font-medium text-text-primary-light dark:text-text-primary-dark mb-1">
                              {evidence.name}
                            </h5>
                            <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mb-2">
                              {evidence.description}
