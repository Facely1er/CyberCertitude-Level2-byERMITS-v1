  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        {/* Modal */}
        <div className="relative bg-surface-light dark:bg-surface-dark rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b border-support-light dark:border-support-dark">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-semibold text-text-primary-light dark:text-text-primary-light">
                Keyboard Shortcuts
              </h2>
            </div>
            <button
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex-1">
                <input
                  ...
                  className="w-full px-3 py-2 border border-support-light dark:border-support-dark rounded-md bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-light placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500"
                />
              </div>
              ...
              <div className="sm:w-48">
                <select
                  ...
                  className="w-full px-3 py-2 border border-support-light dark:border-support-dark rounded-md bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-light focus:outline-none focus:ring-2 focus:ring-primary-500"
                >
            ...
            {Object.keys(groupedShortcuts).length === 0 ? (
              ...
              <div className="space-y-6">
                {Object.entries(groupedShortcuts).map(([category, categoryShortcuts]) => (
                  <div key={category}>
                    <h3 className="text-lg font-medium text-text-primary-light dark:text-text-primary-light mb-3">
                      {category}
                    </h3>
                    <div className="space-y-2">
                      {categoryShortcuts.map((shortcut, index) => (
                        <div
                          key={index}
                          className="flex items-center justify-between py-2 px-3 rounded-md hover:bg-background-light dark:hover:bg-surface-dark"
                        >
                          <span className="text-sm text-text-primary-light dark:text-text-secondary-dark">
                            {shortcut.description}
                          </span>
                          <div className="flex items-center space-x-1">
                            {shortcut.keys.map((key, keyIndex) => (
                              <React.Fragment key={keyIndex}>
                                <kbd className="inline-flex items-center px-2 py-1 text-xs font-medium text-text-primary-light dark:text-text-primary-light bg-support-light dark:bg-primary-600 border border-support-light dark:border-support-light rounded">
                                  {renderKey(key)}
                                </kbd>
          ...
          {/* Footer */}
          <div className="flex items-center justify-between p-6 border-t border-support-light dark:border-support-dark bg-background-light dark:bg-surface-dark">
            <div className="text-sm text-text-muted-light dark:text-text-muted-dark">
              Press <kbd className="px-1 py-0.5 text-xs font-medium text-text-primary-light dark:text-text-primary-light bg-support-light dark:bg-primary-600 border border-support-light dark:border-support-light rounded">Esc</kbd> to close
            </div>
            <button
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium text-text-primary-light dark:text-text-secondary-dark bg-surface-light dark:bg-primary-600 border border-support-light dark:border-support-light rounded-md hover:bg-background-light dark:hover:bg-support-light focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
              Close
            </button>
          </div>
