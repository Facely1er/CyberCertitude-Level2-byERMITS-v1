 ...
 */
export const upsertProfile = async (profileData: any): Promise<{ data: any; error: any }> => {
  if (!isSupabaseReady || !supabase) {
    return {
      data: null,
      error: { message: 'Supabase not configured' }
 ...
 */
export const refreshSession = async (): Promise<{ data: any; error: any }> => {
  if (!isSupabaseReady || !supabase) {
    return {
      data: null,
      error: { message: 'Supabase not configured' }
