...
export class AuditLogger {
  ...
  private persistToLocalStorage(): void {
    try {
      ...
      logger.error('Failed to persist audit logs to localStorage:', error);
    }
  }
  ...
  private async persistToDatabase(entry: AuditLogEntry): Promise<void> {
    ...
    if (import.meta.env.DEV) {
      logger.log('Audit log entry (localStorage only):', entry);
    }
  }
  ...
  loadFromLocalStorage(): void {
    try {
      ...
      logger.error('Failed to load audit logs from localStorage:', error);
    }
  }
}
